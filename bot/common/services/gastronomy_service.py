...
class GastronomyService:
    ...
    async def get_list(self, page: int = 1) -> Optional:
    ...
    async def create(self, data: Dict, telegram_id: int) -> Optional:
    ...
    async def update(self, item_id: int, data: Dict, telegram_id: int) -> Optional:
        user_info = await get_user_info(telegram_id)
        response = await make_request(
            sub_url=f"{self.prefix}{item_id}/",

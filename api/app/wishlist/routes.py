...
async def get_cart(
    ...
    return await get_wishlist_items(session=session, user_id=current_user.id)
...
async def post_cart_item(
    ...
    return await add_to_wishlist(session=session, user_id=current_user.id, new_item=new_item)
...
async def delete_cart_item(
    ...
    await remove_wishlist_item(session=session, user_id=current_user.id, item_id=item_id)
